...
# Description: gem5 configuration script for CVA6 RTL memory test simulation with various memory backends.
# It automatically compiles the memtest.S assembly and configures the memory controller
# (SimpleMemory, DDR3, DDR4, LPDDR2, HBM) to compare timing statistics.
...
parser = argparse.ArgumentParser(description="Run CVA6 RTL memory test in gem5 with various memory configurations")
parser.add_argument("--num-words", type=int, default=50,
                    help="Number of 64-bit words to read and write (default: 50)")
parser.add_argument("--use-l2", action="store_true",
                    help="Enable L2 cache in the simulation")
parser.add_argument("--mem-type", type=str, default="simple",
                    choices=["simple", "ddr3", "ddr4", "lpddr2", "hbm"],
                    help="Type of memory controller to simulate (default: simple)")
parser.add_argument("--simple-latency", type=str, default="10ns",
                    help="Latency for SimpleMemory if --mem-type=simple (default: 10ns)")
...
# Set up physical memory and connect it to the crossbar based on --mem-type
mem_range = system.mem_ranges[0]

if args.mem_type == "simple":
    print(f"Instantiating SimpleMemory with latency={args.simple_latency}...")
    system.mem_ctrl = SimpleMemory(range=mem_range, latency=args.simple_latency)
elif args.mem_type == "ddr3":
    print("Instantiating MemCtrl with DDR3_1600_8x8...")
    system.mem_ctrl = MemCtrl(dram=DDR3_1600_8x8(range=mem_range))
elif args.mem_type == "ddr4":
    print("Instantiating MemCtrl with DDR4_2400_8x8...")
    system.mem_ctrl = MemCtrl(dram=DDR4_2400_8x8(range=mem_range))
elif args.mem_type == "lpddr2":
    print("Instantiating MemCtrl with LPDDR2_S4_1066_1x32...")
    system.mem_ctrl = MemCtrl(dram=LPDDR2_S4_1066_1x32(range=mem_range))
elif args.mem_type == "hbm":
    print("Instantiating MemCtrl with HBM_1000_4H_1x128...")
    system.mem_ctrl = MemCtrl(dram=HBM_1000_4H_1x128(range=mem_range))
...
# Run simulation loop checking for tohost write or ebreak
...
print("\n============================================")
if tohost_val == 1 or exit_cause == "CVA6 program hit ebreak instruction":
    print(f"CVA6 Memory Test finished! exit_cause: {exit_cause}")
    print(f"SUCCESS: Successfully wrote and verified {args.num_words} words in RAM!")
    sys.exit(0)
elif tohost_val != 0 or "ebreak" in exit_cause:
    print(f"CVA6 Memory Test finished! tohost = {tohost_val}, exit_cause: {exit_cause}")
